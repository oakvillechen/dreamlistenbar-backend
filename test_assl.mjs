import CryptoJS from 'crypto-js';

const assl = '';
const ts = '44241f74a27f1774464176';
const es = '60';

const YUETINGBA_KEY = CryptoJS.enc.Base64.parse('le95G3hnFDJsBE+1/v9eYw==');
const YUETINGBA_IV = CryptoJS.enc.Base64.parse('IvswQFEUdKYf+d1wKpYLTg==');

console.log('--- OLD KEY ---');
try {
    const decrypted = CryptoJS.AES.decrypt(assl, YUETINGBA_KEY, {
        iv: YUETINGBA_IV,
        mode: CryptoJS.mode.CBC,
        padding: CryptoJS.pad.Pkcs7
    }).toString(CryptoJS.enc.Utf8);
    console.log('Decrypted Old:', decrypted.substring(0, 100));
} catch (e) {
    console.log('Old failed');
}

console.log('--- DYNAMIC KEY (ts) ---');
try {
    const key = CryptoJS.enc.Utf8.parse(ts.substring(0, 16));
    const iv = CryptoJS.enc.Utf8.parse(es.padEnd(16, '0'));
    const decrypted = CryptoJS.AES.decrypt(assl, key, {
        iv: iv,
        mode: CryptoJS.mode.CBC,
        padding: CryptoJS.pad.Pkcs7
    }).toString(CryptoJS.enc.Utf8);
    console.log('Decrypted Dynamic:', decrypted.substring(0, 100));
} catch (e) {
    console.log('Dynamic failed');
}
