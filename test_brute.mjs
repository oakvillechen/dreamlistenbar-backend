import CryptoJS from 'crypto-js';

const assl = '';
const ts = '44241f74a27f1774464176';
const es = '60';
const salt = 'le95G3hnFDJsBE+1/v9eYw==';

function tryDecrypt(keyStr, ivStr) {
    const key = CryptoJS.enc.Utf8.parse(keyStr.substring(0, 16));
    const iv = CryptoJS.enc.Utf8.parse(ivStr.substring(0, 16).padEnd(16, '0'));
    try {
        const decrypted = CryptoJS.AES.decrypt(assl, key, {
            iv: iv,
            mode: CryptoJS.mode.CBC,
            padding: CryptoJS.pad.Pkcs7
        }).toString(CryptoJS.enc.Utf8);
        return decrypted;
    } catch (e) {
        return null;
    }
}

console.log('--- Brute Force ---');
// 1. Direct ts/es
console.log('1. ts/es:', tryDecrypt(ts, es).substring(0, 50));

// 2. MD5 derived (like efi)
const derivedKey = CryptoJS.MD5(ts + es + salt).toString().substring(0, 16);
const derivedIV = CryptoJS.MD5(es + ts + salt).toString().substring(0, 16);
console.log('2. MD5(ts+es+salt):', tryDecrypt(derivedKey, derivedIV).substring(0, 50));

// 3. Just ts as key, IV as ts last 16
console.log('3. ts/ts_last16:', tryDecrypt(ts, ts.substring(ts.length - 16)).substring(0, 50));

// 4. salt as key, ts as IV
console.log('4. salt/ts:', tryDecrypt(salt, ts).substring(0, 50));
